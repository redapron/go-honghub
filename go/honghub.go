	...
	for _, event := range rs.Event {
		...
		if event.Message.Type == "text" {
			...
			if strings.Index(event.Message.Text, "นุ่น") >= 0 {
				...
				template := linebot.NewConfirmTemplate("สวัสดีค่ะ , นุ่นคือบอทที่จะช่วยให้คุณจองห้องประชุมได้ง่ายๆ ^^", leftBtn, rightBtn)
				message = linebot.NewTemplateMessage("นุ่นกล่าวทักทาย", template)
			} else if strings.Index(event.Message.Text, "จองห้องหน่อยสิ") >= 0 {
				message = linebot.NewTextMessage("ไม่ทราบว่าต้องการประชุมที่ตึกไหนคะ").
			...
			} else if strings.Index(event.Message.Text, "KBTG") >= 0 || strings.Index(event.Message.Text, "KBTG") >= 0 {
				message = linebot.NewTextMessage("กรุณาเลือกประเภทห้องประชุมค่ะ ").
					WithQuickReplies(linebot.NewQuickReplyItems(
						linebot.NewQuickReplyButton(
							"",
			...
			} else if strings.Index(event.Message.Text, "ห้อง Whiteboard") >= 0 {
				message = linebot.NewTextMessage("ไม่ทราบว่าต้องการจองห้องไหนคะ").
					WithQuickReplies(linebot.NewQuickReplyItems(
						linebot.NewQuickReplyButton(
							"",
			...
			} else if strings.Index(event.Message.Text, "ห้อง 1") >= 0 {
				template := linebot.NewButtonsTemplate(
					"", "", "ช่วงเวลาไหนคะ",
					linebot.NewDatetimePickerAction("เลือกวันเวลา", "DATETIME", "datetime", "", "", ""),
				)
				message = linebot.NewTemplateMessage("ช่วงเวลาไหนคะ", template)

			} else {
				message = linebot.NewTextMessage("การจองห้องสำเร็จเรียบร้อยแล้วค่ะ")
			}
